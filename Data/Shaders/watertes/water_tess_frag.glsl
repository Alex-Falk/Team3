...
void main(void) {
	...
	//calculate tex coord
	//vec2 distortedTexCoords = texture(dudvMap, vec2(IN.texCoord.x + moveFactor/5, IN.texCoord.y)).rg*0.1;
	vec2 distortedTexCoords = IN.texCoord + vec2(moveFactor/10, 0);
	//vec2 totalDistortion = (texture(dudvMap, distortedTexCoords).rg) * waveStrength;

	vec3 normal = normalize(IN.normal);
	vec4 texColor = texture(uDiffuseTex, distortedTexCoords);
	...
	vec3 nor = IN.normal;
	nor.x += distortedTexCoords.x*2.0;
	nor.y += distortedTexCoords.y*2.0;
	nor.z += distortedTexCoords.y*2.0;
	vec4 reflection = texture(cubeTex, reflect(toWPos, normalize(nor)));
